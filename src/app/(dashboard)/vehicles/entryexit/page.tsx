...
import { getVehicleImage, getEntryExitData } from '@/app/server/action' // Assuming this is where you exported it

// Define the type based on your SQL schema
export type VehicleEventRecord = {
  id: number
...
const columnHelper = createColumnHelper<VehicleEventRecord>()

// const defaultData: VehicleEventRecord[] = await getEntryExitData()

const VehicleEventTable = () => {
  // States
  const [data, setData] = useState<VehicleEventRecord[]>([])
  ...
  const handleViewImage = async (imageId: number) => {
    setLoadingImg(true)
    setIsModalOpen(true)
    const base64 = await getVehicleImage(imageId.toString())

    setImgUrl(base64)
    setLoadingImg(false)
  }

  const columns = useMemo<ColumnDef<VehicleEventRecord, any>[]>(
    () => [
      ...
      columnHelper.accessor('movement', {
        ...
        cell: ({ row }) => (
          <Chip
            label={row.original.movement === '1' ? 'ENTRY' : row.original.movement === '2' ? 'EXIT' : 'UNKNOWN'}
            variant='outlined'
            size='small'
            color={row.original.movement === '1' ? 'success' : row.original.movement === '2' ? 'primary' : 'default'}
          />
        )
      }),
      columnHelper.accessor('created_at', {
        header: 'Timestamp',
        cell: ({ row }) => (
          <Typography>{row.original.created_at ? formatTimestamp(row.original.created_at) : 'N/A'}</Typography>
        )
      }),
  ...
  return (
    <Card>
      <CardHeader title='Entry Exit Events Log' />
